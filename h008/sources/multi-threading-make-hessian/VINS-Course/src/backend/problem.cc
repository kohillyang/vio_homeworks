...
bool Problem::Solve(int iterations) {
    ...
    FILE *f_hessian_cost = fopen("hessian_cost.txt", "at");
...
//        if (sqrt(currentChi_) < 1e-15)
    ...
    fprintf(f_hessian_cost, "%f\r\n", t_hessian_cost_);
    fclose(f_hessian_cost);
    t_hessian_cost_ = 0.;
    return true;
}
...
void Problem::MakeHessian() {
    ...
    std::vector<std::shared_ptr<Edge>> edges_vector;
    for(auto &edge:edges_){
        edges_vector.push_back(edge.second);
    }
#pragma omp parallel for
    for (int i=0; i< edges_vector.size(); i++) {
        auto edge = edges_vector[i];
...
#pragma omp critical
            ...
            MatXX tmp = drho * jacobian_i.transpose()* edge->Information() * edge->Residual();
            for(int b_i=0; b_i < dim_i; b_i ++ ){
#pragma omp atomic updat/e
                b[index_i + b_i] -= tmp(b_i);
            }
