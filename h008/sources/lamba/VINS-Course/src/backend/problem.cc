...
//        TicToc t_Hmminv;
    	...
    	MatXX p0 = method0_Lambda_ * Hessian_.diagonal().asDiagonal();
    	MatXX Hesian_plus_lambda = Hessian_ + p0;
        int reserve_size = ordering_poses_;
        int marg_size = ordering_landmarks_;
        MatXX Hmm = Hesian_plus_lambda.block(reserve_size, reserve_size, marg_size, marg_size);
        MatXX Hpm = Hesian_plus_lambda.block(0, reserve_size, reserve_size, marg_size);
        MatXX Hmp = Hesian_plus_lambda.block(reserve_size, 0, marg_size, reserve_size);
        ...
        H_pp_schur_ = Hesian_plus_lambda.block(0, 0, ordering_poses_, ordering_poses_) - tempH * Hmp;
...
//    }
	// do nothing here
	// we copy a new Hessian when solving norm equation.
}
...
//    scale = 0.5* delta_x_.transpose() * (currentLambda_ * delta_x_ + b_);
    scale = .5 * (delta_x_.transpose() * (method0_Lambda_ * Hessian_.diagonal().asDiagonal() * delta_x_ + b_))[0];
    ...
    if (rho > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        ...
        ni_ = 2;
        currentChi_ = tempChi;
        method0_Lambda_ = std::max(method0_Lambda_ / 9, 1e-7);
        return true;
    } else {
        currentLambda_ *= ni_;
        ni_ *= 2;
        method0_Lambda_ = std::min(method0_Lambda_ * 11, 10e7);
        return false;
    }
}
