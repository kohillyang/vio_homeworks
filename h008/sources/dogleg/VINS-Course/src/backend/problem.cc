...
double GetInfiniteNorm(const VecX &in1){
	double r = 0;
	for(int i=0; i< in1.size(); i++){
		double x = std::abs(in1(i, 0));
		r = std::max(x, r);
	}
	return r;
}
...
bool Problem::Solve(int iterations) {
    ...
    auto decide_found = [&](){return std::sqrt(this->GetChiSquaredNormL2()) <= this->epison_3 || GetInfiniteNorm(b_) <= this->epison_1;};
//    bool found = std::sqrt(this->GetChiSquaredNormL2()) <= this->epison_3 || GetInfiniteNorm(b_) <= this->epison_1;
    bool found = decide_found();
    trust_delta = 1;
    double dog_leg_gain_ratio_rho = 0.0;
    while(found == false && (iter < iterations)){
    	currentChi_ = std::sqrt(this->GetChiSquaredNormL2());
		std::cout << "iter: " << iter << " , chi= " << currentChi_ << " , trust_delta= " << trust_delta << ", rho= " << dog_leg_gain_ratio_rho << std::endl;
//    	iter ++;
    	double alpha = double(g_.squaredNorm()) / (g_.transpose() * Hessian_ * g_);
//        double alpha = double(g_.squaredNorm() / Jb_squared_norm_);
    	VecX h_sd = alpha * -1 * g_;
    	SolveLinearSystem();
    	VecX h_gn = this->delta_x_;
    	VecX h_dl;
    	double L0_sub_Ldl;
    	if(h_gn.norm() <= this->trust_delta){
    	    cerr << "case 1" << endl;
    		h_dl = h_gn;
//    		L0_sub_Ldl = .5 * this->GetChiSquaredNormL2();
    	}else if(alpha *h_sd.norm() >= this->trust_delta){
    	    cerr << "case 2" << endl;
    		h_dl = this->trust_delta / h_sd.norm() * h_sd;
//    		L0_sub_Ldl = this->trust_delta * (2 * (alpha * g_).norm() - trust_delta) / 2 / alpha;
    	}else{
    	    cerr << "case 3 " << endl;
    		// Calc beta
    		VecX a = alpha * h_sd;
    		VecX b_sub_a = h_gn - alpha * h_sd;
    		double c = a.transpose() * b_sub_a;
        	double beta = 0;
    		if(c <= 0){
    			beta = (-c + std::sqrt(c * c + b_sub_a.squaredNorm() * (trust_delta * trust_delta - a.squaredNorm()))) / b_sub_a.squaredNorm();
    		}else{
    			beta = (trust_delta * trust_delta - a.squaredNorm()) / (c + std::sqrt(c *c + b_sub_a.squaredNorm() * (trust_delta * trust_delta-a.squaredNorm())));
    		}
    		h_dl = a + beta * b_sub_a;
//    		L0_sub_Ldl = .5 * alpha * (1-beta)*(1-beta) * g_.squaredNorm() + beta * (2-beta) * currentChi_;
            cerr << "beta: " << beta << "c:" << c << endl;
    	}
        double L0_sub_Ldl0 = h_dl.transpose() * g_ ;//+ .5 * (h_dl.transpose() * Hessian_ * h_dl);
        double L0_sub_Ldl1 = h_dl.transpose() * Hessian_ * h_dl;
        L0_sub_Ldl = L0_sub_Ldl0 + L0_sub_Ldl1;
        L0_sub_Ldl *= -1;
        cerr << L0_sub_Ldl <<" " << h_dl.norm() << " " << g_.norm() << endl;
    	if (h_dl.norm() <= epison_2 * (std::sqrt(this->GetXSquaredNormL2()) + epison_3)){
    		found = true;
    	}else{
    		double tmpChi = this->GetChiSquaredNormL2();
    		delta_x_ = h_dl;
    		this->UpdateStates();
    		double Chi2 = this->GetChiSquaredNormL2();
    		dog_leg_gain_ratio_rho = .5 * (tmpChi - Chi2) / L0_sub_Ldl;
    		if(dog_leg_gain_ratio_rho > 0){
    			iter ++;
    			MakeHessian();
    			found = decide_found();
    		}else{
    		...
    		if(dog_leg_gain_ratio_rho > 0.75){
    			trust_delta = std::max(trust_delta, 3 * h_dl.norm());
    		} else if(dog_leg_gain_ratio_rho < 0.25){
    			trust_delta /= 2;
    			found = trust_delta <= epison_2 * (std::sqrt(this->GetXSquaredNormL2()) + epison_2);
    		}
    	}
    }
    return true;
...
//    VecX Jb(VecX::Zero(size));
    double Jb_squared_norm = 0;
...
//            std::cout << drho * jacobian_i.transpose()* edge.second->Information() * edge.second->Residual() << endl;
            b.segment(index_i, dim_i).noalias() -= drho * jacobian_i.transpose()* edge.second->Information() * edge.second->Residual();
            Jb_squared_norm += (drho * jacobian_i * jacobian_i.transpose()* edge.second->Information() * edge.second->Residual()).squaredNorm();
        }
...
//                std::cout << " fixed prior, set the Hprior and bprior part to zero, idx: "<<idx <<" dim: "<<dim<<std::endl;
    ...
    g_ = -1 * b; // J(x)^T \times f(x)
    Jb_squared_norm_ = Jb_squared_norm;
...
double Problem::GetXSquaredNormL2(){
	double r = 0;
    for (auto vertex: verticies_) {
        r += vertex.second->Parameters().squaredNorm();
    }
    return r;
}
...
double Problem::GetChiSquaredNormL2(){
    double tempChi = 0.0;
    for (auto edge: edges_) {
        edge.second->ComputeResidual();
