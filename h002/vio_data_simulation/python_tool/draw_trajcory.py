...
position_mid_point = data[:,[tx_index, tx_index + 1, tx_index + 2]]

data = np.loadtxt(filepath + '/imu_int_pose_euler_noise.txt')
# timestamp1 = data[:,0]
# quaterntions1 = data[:,[tx_index + 6, tx_index + 3, tx_index + 4, tx_index + 5]] # qw,qx,qy,qz
position_euler = data[:,[tx_index, tx_index + 1, tx_index + 2]]
...
ax.plot(position_euler[:,0], position_euler[:,1], position_euler[:,2], 'c', label='imu_int_euler')
ax.plot(position_mid_point[:,0], position_mid_point[:,1], position_mid_point[:,2], 'm', label='imu_int_midpoint', )
