...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 0; i < imudata.size() - 1; ++i) {
        ...
        auto f_omega_euler = [&imudata](int k) -> Eigen::Vector3d {return imudata[k].imu_gyro;};
        auto f_a_euler = [&imudata, &Qwb, &gw, &f_omega_euler, &dt](int k) -> std::pair<Eigen::Vector3d, Eigen::Quaterniond>{
            Eigen::Vector3d dtheta_half = f_omega_euler(k) * dt /2.0;
            ...
            dq.z() = dtheta_half.z();
            dq.normalize();
            Eigen::Quaterniond q_wb_plus1 = Qwb * dq;
            auto a_k = Qwb * imudata[k].imu_acc  + gw;
            return {a_k, q_wb_plus1};
        };
        auto f_omega_mid_point = [&imudata](int k) -> Eigen::Vector3d {return .5 *(imudata[k].imu_gyro + imudata[k + 1].imu_gyro);};
        auto f_a_mid_point = [&imudata, &Qwb, &gw, &f_omega_mid_point, &dt](int k) -> std::pair<Eigen::Vector3d, Eigen::Quaterniond>{
            Eigen::Vector3d dtheta_half = f_omega_mid_point(k) * dt /2.0;
            Eigen::Quaterniond dq;
            dq.w() = 1;
            dq.x() = dtheta_half.x();
            dq.y() = dtheta_half.y();
            ...
            dq.normalize();
            Eigen::Quaterniond q_wb_plus1 = Qwb * dq;
            auto a_k = .5 * (Qwb * imudata[k].imu_acc + q_wb_plus1 * imudata[k + 1].imu_acc)  + gw;
            return {a_k, q_wb_plus1};
        };

        /// imu 动力学模型 欧拉积分
        auto r = f_a_mid_point(i);
        Eigen::Vector3d acc_w = r.first;  // aw = Rwb * ( acc_body - acc_bias ) + gw
        Qwb = r.second;
